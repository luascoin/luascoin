...
`test/functional/test_framework/test_framework.py:LuascoinTestFramework.main`.
...
def main():
    ...
    enable_luascoind = config["components"].getboolean("ENABLE_LUASCOIND")

    if not enable_luascoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control):
    ...
    # Warn if luascoind is already running
    # pidof might fail or return an empty string if luascoind is not running
    try:
        if subprocess.check_output(["pidof", "luascoind"]) not in [b'']:
            print("%sWARNING!%s There is already a luascoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
