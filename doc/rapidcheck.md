# RapidCheck property-based testing for Luascoin Core

## Concept

Property-based testing is experimentally being added to Luascoin Core with
...
If RapidCheck is installed, Luascoin Core will automatically run the
...
3. Configure Luascoin Core with RapidCheck.

    `cd` to the directory of your local luascoin repository and run
...
4. Build Luascoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_luascoin-crypto_gen.o
    CXX      test/test_luascoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Luascoin Core.
