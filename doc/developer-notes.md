...
- [Development guidelines](#development-guidelines)
    - [General Luascoin Core](#general-luascoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Luascoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play luascoins" on the test network, if you
...
Luascoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/luascoin/luascoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_luascoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_luascoin --log_level=test_suite
$ valgrind -v --leak-check=full src/luascoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_luascoin.coverage/index.html`.
```
...
To profile a running luascoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep luascoind` -- sleep 60
```
...
Luascoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/luascoin/luascoin/issues/12691)
...
- ThreadSocketHandler : Sends/Receives data from peers on port 10153.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `LUASCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef LUASCOIN_FOO_BAR_H
#define LUASCOIN_FOO_BAR_H
...
#endif // LUASCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of Luascoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Luascoin Core so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/luascoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/luascoin-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/luascoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Luascoin's `select()` loop, because
...
compatibility between nodes. This happened in Luascoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Luascoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/luascoin/luascoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/luascoin/luascoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/luascoin/luascoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `luascoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `luascoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Luascoin Core, the wallet was always
