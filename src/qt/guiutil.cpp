...
#include <qt/luascoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Luascoin address (e.g. %1)").arg(
...
bool parseLuascoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!LuascoinUnits::parse(LuascoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatLuascoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("luascoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(LuascoinUnits::format(LuascoinUnits::BTC, info.amount, false, LuascoinUnits::separatorNever));
        paramCount++;
    }
...
bool openLuascoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", LUASCOIN_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Luascoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Luascoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Luascoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Luascoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "luascoin.desktop";
    return GetAutostartDir() / strprintf("luascoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a luascoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Luascoin\n";
        else
            optionFile << strprintf("Name=Luascoin (%s)\n", chain);
