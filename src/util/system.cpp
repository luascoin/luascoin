 ...
 * https://github.com/luascoin/luascoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //luascoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Luascoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Luascoin
    // Mac: ~/Library/Application Support/Luascoin
    // Unix: ~/.luascoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Luascoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Luascoin";
#else
    // Unix
    return pathRet / ".luascoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LUASCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Luascoin Core copyright is not removed by accident
    if (copyright_devs.find("Luascoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Luascoin Core developers";
    }
