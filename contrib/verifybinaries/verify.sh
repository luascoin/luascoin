...
###   luascoincore.org and luascoin.org and compares them.
...
WORKINGDIR="/tmp/luascoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://luascoincore.org"
HOST2="https://luascoin.org"
BASEDIR="/bin/"
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "luascoin.org failed to provide signature file, but luascoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "luascoin.org and luascoincore.org signature files were not equal?"
